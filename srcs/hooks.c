...
/*   Updated: 2024/02/02 23:23:25 by caredua3         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_is_collectable(t_game *game, int x, int y)
{
	...
	while (i < game->num_of_collect)
	{
			...
			&& game->image.collect.img->instances[i].y == y)
		{
				...
				game->image.collect.img->instances[i].x = 0;
				game->image.collect.img->instances[i].y = 0;
				game->num_collect--;
		}
		i++;
	}
...
void	hooks(mlx_key_data_t keydata, t_game *game)
{
	...
	else if ((keydata.key == MLX_KEY_UP || keydata.key == MLX_KEY_W) && keydata.action == MLX_PRESS)
		player_movement(game, x, y - 64);
	else if ((keydata.key == MLX_KEY_DOWN || keydata.key == MLX_KEY_S) && keydata.action == MLX_PRESS)
		player_movement(game, x, y + 64);
	else if ((keydata.key == MLX_KEY_LEFT || keydata.key == MLX_KEY_A) && keydata.action == MLX_PRESS)
		player_movement(game, x - 64, y);
	else if ((keydata.key == MLX_KEY_RIGHT || keydata.key == MLX_KEY_D) && keydata.action == MLX_PRESS)
		player_movement(game, x + 64, y);
}
